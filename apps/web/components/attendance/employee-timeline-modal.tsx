...
import {
  ...
  Clock, Wifi, Activity, CheckCircle, MapPin, Zap, FileJson, FileText,
} from 'lucide-react';
...
export function EmployeeTimelineModal({ employeeId, employeeName, department = 'Engineering', onClose }: EmployeeTimelineModalProps) {
  ...
  // Use local date string (IST-aware) so session_date lookup is always correct
  const TODAY_STR = (() => {
    const d = new Date();
    return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
  })();

  /** Build all plausible employee_id variants to cover padded/unpadded/UUID mismatches */
  const buildIdVariants = (id: string, name?: string): string[] => {
    const variants = new Set<string>([id]);
    // Numeric part → EMP-01, EMP-1, EMP-000001
  ...
  useEffect(() => {
    const tick = () => setCurrentTime(new Date().toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true }));
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Build OR filter covering all ID variants + name
      ...
      // ── Attendance session (try all ID variants) ────────────────────────────
      ...
      if (sessRows?.[0]) {
        ...
        // Fallback: attendance_records (try all ID variants)
        const recOrFilter = idVariants.map(v => `employee_id.eq.${v}`).join(',');
        const { data: recRows } = await supabase.from('attendance_records').select('*')
          .or(recOrFilter).eq('date', 'Today').limit(1);
        if (recRows?.[0]) {
          const r = recRows[0];
          resolvedCheckIn = r.check_in_time;
          resolvedCheckOut = r.check_out_time;
          setSession(prev => ({ ...prev, checkIn: r.check_in_time, checkOut: r.check_out_time, status: r.status === 'late' ? 'LATE' : 'PRESENT', device: r.device_name || prev.device }));
        }
      }

      // ── Attendance events (all ID variants + name) ──────────────────────────
      ...
      (aeData || []).forEach((row: any) => {
        ...
        const fmt = dateObj.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true });
        ...
        } else { rawEvts.push(item); }
      });

      // If session check_in is still null but we have a CHECK_IN event, use that
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-6 bg-slate-950/80 backdrop-blur-xl" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="w-full sm:max-w-2xl bg-[#0c0e14] border border-slate-800/50 sm:rounded-2xl shadow-2xl flex flex-col max-h-[92vh] sm:max-h-[84vh] overflow-hidden">
          ...
          <div className="p-5 space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-[10px] text-slate-700 font-mono">Realtime · Auto-sync</span>
              </div>
              {loading ? (
                ...
                <div className="relative pl-7">
                  ...
                  <div className="space-y-0.5">
                    {businessEvents.map((evt, idx) => {
                      ...
                      return (
                        <div key={evt.id} className={clsx('relative flex items-center gap-3 py-2.5 px-3 rounded-lg transition-colors duration-200', isLast ? 'bg-slate-900/50' : 'hover:bg-slate-900/30')}>
            ...
            <div className="border border-slate-800/40 rounded-xl overflow-hidden">
              <button onClick={() => setDevOpen(v => !v)} className="w-full flex items-center justify-between px-4 py-2.5 text-left hover:bg-slate-900/30 transition-colors">
                <div className="flex items-center gap-2"><Zap className="w-3 h-3 text-slate-700" /><span className="text-[11px] font-medium text-slate-600 uppercase tracking-wide">Developer Events</span><span className="text-[10px] text-slate-700 bg-slate-800/50 px-1.5 py-0.5 rounded font-mono">{rawEvents.length} TCP</span></div>
                {devOpen ? <ChevronDown className="w-3.5 h-3.5 text-slate-700" /> : <ChevronRight className="w-3.5 h-3.5 text-slate-700" />}
              </button>
              {devOpen && (
                <div className="border-t border-slate-800/40 bg-slate-950/60 max-h-48 overflow-y-auto">
                  {rawEvents.length === 0 ? (<p className="text-[11px] text-slate-700 p-4 text-center">No raw TCP events for today.</p>) : (
                    <div className="divide-y divide-slate-800/30">
                      {rawEvents.map(evt => (
                        <div key={evt.id} className="px-4 py-2 flex items-center justify-between gap-3">
                          <div className="min-w-0"><span className="text-[10px] font-mono text-slate-600 truncate block">{evt.id}</span><span className="text-[10px] text-slate-700">{evt.eventType} · {evt.device}</span></div>
                          <span className="text-[10px] font-mono text-slate-700 shrink-0">{evt.formattedTime}</span>
                        </div>
                      ))}
                    </div>
                  )}
        ...
        <div className="px-5 py-3 border-t border-slate-800/50 bg-[#0c0e14] flex items-center justify-between gap-3 shrink-0">
          <span className="text-[11px] text-slate-700">{businessEvents.length} event{businessEvents.length !== 1 ? 's' : ''} · {rawEvents.length} TCP</span>
          <div className="flex items-center gap-2">
